...
class Robot:public Fighter{
	...
	public:
	Robot(int bonus_damage, double energy, double origenergy, string title, int hitPoints, int maxhitPoints, int strength, int speed, int maxspeed, int magic);
	~Robot();
	 ...
	 *	equal to (strength  * ((current_energy/maximum_energy)^4)).
	 *	Can only be used if the Robot has at least [ROBOT_ABILITY_COST] energy.
	 *	Decreases the Robot's current energy by [ROBOT_ABILITY_COST] (after
	 *	calculating the additional damage) when used.
	 *		Examples:
